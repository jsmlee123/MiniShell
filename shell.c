...
void dopipe(strarray_t *input, strarray_t* last,int loc) {
    ...
    for(int i = 0; i < loc; i++) {
	firstarr[i] = (char*)malloc(sizeof(char) * (strlen(input->strings[i]) + 1));
	...
	secondarr[i-loc-1] = (char*)malloc(sizeof(char*)*(strlen(input->strings[i])+1));
...
int main(int argc, char** argv){
  ...
  while(flag) {
      ...
      if(childID == 0) {
	    ...
	    //if we called cd, then we reach here since cd
	    //doesn't exit the process but returns
	    //we can get the child dir and pipe it to 
	    //the parent process
  ...
  //if we're here, signal was called
  //we free last so nothing leaks
  freeStrArr(last);
  return 0;
}
